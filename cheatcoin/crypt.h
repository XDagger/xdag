...
/* create a new pair of private and public keys; returns a pointer to its internal representation, the private key * saves to the privkey array, the public key to the pubkey array, the parity of the public key to the variable pubkey_bit */
extern void *cheatcoin_create_key(cheatcoin_hash_t privkey, cheatcoin_hash_t pubkey, uint8_t *pubkey_bit);

/* returns the internal representation of the key and the public key for the known private key */
extern void *cheatcoin_private_to_key(const cheatcoin_hash_t privkey, cheatcoin_hash_t pubkey, uint8_t *pubkey_bit);

/* Returns the internal representation of the key to a known public key */
