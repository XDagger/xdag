    ...
    for (int n = 0; n < nmax; ++n)
    {
      ...
      if (t->fds[n].revents &POLLIN)
      {
        ...
        if (conn->packets_in >= FIRST_NSECTORS)
        {
          ...
          {
            xs->head.ttl = ttl;
            dnet_send_xdag_packet(xs->word, &(struct send_parameters)
            {
              conn, 0, 1, 0
            });
          }
...
static void *accept_thread_main(void *arg)
{
  ...
  if (listen(fd, MAX_CONNECTIONS_PER_THREAD))
  { // "1" is the maximal length of the queue
    return 0;
  }
...
int dnet_execute_command(const char *cmd, void *fileout)
{
  ...
  else if (!strcmp(str, "conn"))
  {
    ...
    for (i = 0; i < g_nthreads; ++i)
    {
      for (j = 0; j < g_threads[i].nconnections; ++j)
      {
  ...
  else if (!strcmp(str, "connect"))
  {
    ...
    {
      close(fd);
      fprintf(f, "connect: error adding the connection (ip=%08x, port=%d)\n",
        htonl(peeraddr.sin_addr.s_addr), htons(peeraddr.sin_port));
      return -1;
    }
  }
...
void dnet_for_each_conn(void *(*callback)(void *, void *), void *data)
{
  ...
  {
    for (int i = 0; i < g_nthreads; ++i)
      for (int j = 0; j < g_threads[i].nconnections; ++j)
      {
