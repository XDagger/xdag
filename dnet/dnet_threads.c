	...
	if (t->type == DNET_THREAD_SERVER || t->type == DNET_THREAD_FORWARD_FROM || proto == DNET_UDP) {
        ...
        for(;;) {
            ...
            *t1 = *t;
			...
			ip = ntohl(peeraddr.sin_addr.s_addr);
			ipgood = (ip == 0x34052044 || ip == 0xC7101f60 || ip == 0xCA61FBE5 || ip == 0x6441ADC2);
			if (g_n_inbound >= MAX_N_INBOUND && !ipgood) { close(t1->conn.socket); goto begin; }
		...
		if (t->type == DNET_THREAD_STREAM) {
			...
			if (res) { mess = "connection error"; goto err; }
		}
    }
...
static void *dnet_thread_accepted(void *arg) {
    struct dnet_thread *t = (struct dnet_thread *)arg;
    const char *mess;
	int res, ip = t->conn.ipaddr, port = t->conn.port;
