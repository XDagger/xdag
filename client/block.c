...
begin:
    ...
    else
    {
        ...
        for(ref = noref_first; ref && res < XDAG_BLOCK_FIELDS; ref = ref->ref) if(ref->time < send_time)
        {
...
static int request_blocks(xdag_time_t t, xdag_time_t dt)
{
    ...
    if(dt <= REQUEST_BLOCKS_MAX_TIME)
    {
        ...
        for(i = 0; xdag_info("QueryB: t=%llx dt=%llx", t, dt),
            i < QUERY_RETRIES && (res = xdag_request_blocks(t, t + dt, &t0, add_block_callback)) < 0; ++i);
        if(res <= 0)
        {
            return -1;
        }
    ...
    else
    {
        ...
        for(i = 0; xdag_info("QueryS: t=%llx dt=%llx", t, dt),
            i < QUERY_RETRIES && (res = xdag_request_sums(t, t + dt, rsums)) < 0; ++i);
        if(res <= 0)
        {
            return -1;
        }
