...
static uint64_t apply_block(struct block_internal *bi)
{
	...
	for (int i = 0; i < bi->nlinks; ++i) {
		if (1 << i & bi->in_mask) {
			accept_amount(bi->link[i], (cheatcoin_amount_t)0 - bi->linkamount[i]);
		} else {
...
static int valid_signature(const struct cheatcoin_block *b, int signo_r, int nkeys, struct cheatcoin_public_key *keys)
{
	...
	for (i = signo_r; i < CHEATCOIN_BLOCK_FIELDS; ++i) {
		if (cheatcoin_type(b, i) == CHEATCOIN_FIELD_SIGN_IN || cheatcoin_type(b, i) == CHEATCOIN_FIELD_SIGN_OUT) {
			...
			if (i > signo_r && signo_s < 0 && cheatcoin_type(b, i) == cheatcoin_type(b, signo_r)) {
				signo_s = i;
			}
		}
	}
	...
	if (signo_s >= 0) {
		for (i = 0; i < nkeys; ++i) {
			...
			if (!cheatcoin_verify_signature(keys[i].key, hash, b->field[signo_r].data, b->field[signo_s].data)) {
				return i;
			}
		}
	}
...
static int add_block_nolock(struct cheatcoin_block *b, cheatcoin_time_t limit)
{
	...
	if (cheatcoin_type(b, 0) != CHEATCOIN_FIELD_HEAD) {
		i = cheatcoin_type(b, 0);
		err = 1;
		goto end;
	}
	...
	for (i = 1; i < CHEATCOIN_BLOCK_FIELDS; ++i) {
		...
		case CHEATCOIN_FIELD_NONCE:
			break;
		case CHEATCOIN_FIELD_IN:
			inmask |= 1 << i;
			break;
		case CHEATCOIN_FIELD_OUT:
			outmask |= 1 << i;
			break;
		case CHEATCOIN_FIELD_SIGN_IN:
			if (++nsignin & 1) {
				signinmask |= 1 << i;
			}
			break;
		case CHEATCOIN_FIELD_SIGN_OUT:
			if (++nsignout & 1) {
				signoutmask |= 1 << i;
			}
			break;
	...
	if (nsignout) {
		our_keys = cheatcoin_wallet_our_keys(&nourkeys);
	}
	...
	for (i = j = 0; i < nkeys; ++i) {
		if (1 << i & verified_keys_mask) {
			if (i != j) {
				cheatcoin_free_key(public_keys[j].key);
			}
		...
		if (1 << i & (inmask | outmask)) {
			...
			*psum += b->field[i].amount;
			...
			if (MAIN_TIME(ref->time) < MAIN_TIME(bi.time)) {
				diff = cheatcoin_diff_add(diff0, ref->difficulty);
			} else {
				diff = ref->difficulty;
				...
				if (ref && cheatcoin_diff_gt(cheatcoin_diff_add(diff0, ref->difficulty), diff)) {
					diff = cheatcoin_diff_add(diff0, ref->difficulty);
				}
			}

			if (cheatcoin_diff_gt(diff, bi.difficulty)) {
	...
	if (!(theader & (sizeof(struct cheatcoin_block) - 1))) {
		bi.storage_pos = theader;
	} else {
		bi.storage_pos = cheatcoin_storage_save(b);
	}
...
int cheatcoin_create_block(struct cheatcoin_field *fields, int ninput, int noutput, cheatcoin_amount_t fee, cheatcoin_time_t send_time)
{
	...
	if (res0 > CHEATCOIN_BLOCK_FIELDS) {
		return -1;
	}
	...
	if (!send_time) {
		...
		mining = (send_time > get_timestamp() && res0 + 1 <= CHEATCOIN_BLOCK_FIELDS);
	}
 ...
 begin:
	...
	if (g_light_mode) {
		...
		for (ref = noref_first; ref && res < CHEATCOIN_BLOCK_FIELDS; ref = ref->ref) {
	...
	for (j = 0; j < ninput; ++j) {
		setfld(CHEATCOIN_FIELD_IN, fields + j, cheatcoin_hash_t);
	}

	for (j = 0; j < noutput; ++j) {
		setfld(CHEATCOIN_FIELD_OUT, fields + ninput + j, cheatcoin_hash_t);
	}
...
static int request_blocks(cheatcoin_time_t t, cheatcoin_time_t dt)
{
	...
	if (dt <= REQUEST_BLOCKS_MAX_TIME) {
		...
		for (i = 0;
			cheatcoin_info("QueryB: t=%llx dt=%llx", t, dt),
			i < QUERY_RETRIES && (res = cheatcoin_request_blocks(t, t + dt, &t0, add_block_callback)) < 0;
			++i);
		...
		if (cheatcoin_load_sums(t, t + dt, lsums) <= 0) {
			return -1;
		}
		...
		for (i = 0;
			cheatcoin_info("QueryS: t=%llx dt=%llx", t, dt),
			i < QUERY_RETRIES && (res = cheatcoin_request_sums(t, t + dt, rsums)) < 0;
			++i);
...
static void *work_thread(void *arg)
{
 ...
 begin:
	...
	for (;;) {
		...
		if (t > t0) {
			g_cheatcoin_extstats.hashrate_s = ((double)(nhashes - nhashes0) * 1024) / (t - t0);
		}
...
int cheatcoin_blocks_start(int n_mining_threads, int miner_address)
{
	...
	if (g_cheatcoin_testnet) {
		cheatcoin_era = CHEATCOIN_TEST_ERA;
	}
