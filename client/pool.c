...
static void *pool_main_thread(void *arg)
{
 ...
 disconnect_free:
			...
			if (p->revents & POLLERR) {
				done = 1; mess = "socket error"; goto disconnect;
			}
			...
			if (p->revents & POLLIN) {
				...
				if (todo <= 0) {
					mess = "read error"; goto disconnect;
				}
						...
						if (m->block_size == CHEATCOIN_BLOCK_FIELDS) {
							...
							if (crc == crc_of_array((uint8_t*)m->block, sizeof(struct cheatcoin_block))) {
								...
								if (!g_firstb) {
									...
									g_lastb->field[0].transport_header = (uintptr_t)m->block, g_lastb = m->block;
								}
...
static int pay_miners(cheatcoin_time_t t)
{
	...
	for (i = 0; i < nminers; ++i) {
		...
		if (reward_ind < 0 && !memcmp(nonce, m->id.data, sizeof(cheatcoin_hashlow_t))) {
			reward_ind = i;
		}
	}
...
static void *miner_net_thread(void *arg)
{
 ...
 begin:
	...
	if (g_miner_address) {
		if (cheatcoin_address2hash(g_miner_address, hash)) {
			mess = "incorrect miner address"; goto err;
		}
	} else if (cheatcoin_get_our_block(hash)) {
	...
	if (send_to_pool(b.field, CHEATCOIN_BLOCK_FIELDS) < 0) {
...
static void *mining_thread(void *arg)
{
	...
	while (!g_cheatcoin_sync_on && !g_stop_mining) {
		sleep(1);
	}
...
{
	while (!g_cheatcoin_sync_on && !g_stop_general_mining) {
		sleep(1);
	}

	while (!g_stop_general_mining) {
		cheatcoin_create_block(0, 0, 0, 0, cheatcoin_main_time() << 16 | 0xffff);
	}
