...
static int correct_storage_sum(const char *path, int pos, const struct xdag_storage_sum *sum, int add) {
	...
	if (f) {
		...
		f = fopen(path, "wb");
		if (!f) { xdag_err("Storag: can't create file %s", path); return -1; }
...
uint64_t xdag_load_blocks(xdag_time_t start_time, xdag_time_t end_time, void *data, void *(*callback)(void *, void *)) {
	...
	while (start_time < end_time) {
		...
		if (k) qsort(pbuf, k, sizeof(struct xdag_block *), sort_callback);
		for (i = 0; i < k; ++i) {
...
int xdag_load_sums(xdag_time_t start_time, xdag_time_t end_time, struct xdag_storage_sum sums[16]) {
	...
	f = fopen(path, "rb");
	if (f) { fread(buf, sizeof(struct xdag_storage_sum), 256, f); fclose(f); }
	...
	} else memcpy(sums, buf + (start_time >> ((level + 4) * 4) & 0xf0), 16 * sizeof(struct xdag_storage_sum));
	return 1;
}
